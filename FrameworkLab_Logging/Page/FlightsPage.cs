...
namespace FrameworkLab
{
    public class FlightsPage
    {
		...
		private const string url = "https://wizzair.com/";

		[FindsBy(How = How.XPath, Using = "//input[@id='search-departure-station']")]
        private IWebElement departureCity;
        [FindsBy(How = How.XPath, Using = "//input[@id='search-arrival-station']")]
        private IWebElement destinationCity;
		[FindsBy(How = How.XPath, Using = "//input[@id='search-departure-date']")]
		...
		[FindsBy(How = How.XPath, Using = "//div[@class='locations-container__no-results']")] 
		...
		public FlightsPage()
        {
			...
			Logger.Log.Info($"Go to site{url}");
		}
		...
		{
			Logger.Log.Info("Enter departure city (from)");
			departureCity.Clear();
		...
		{
			Logger.Log.Info("Enter destination city (to)");
		...
		{
			Logger.Log.Info("Enter departure date");
			departDate.Clear();
			departDate.SendKeys(date);
			return this;
		}

		public string GetDepartureDateErrorMessage() 
		{
		...
		{
			Logger.Log.Info("Get destination error message from FlightsPage if not exist");
